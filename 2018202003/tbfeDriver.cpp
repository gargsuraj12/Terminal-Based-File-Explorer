...
int gotoSpecificDirectory(string path){
	string gotoPath;
	...
	if(path == "/"){
		...
		int len = path.length();
		string sub = path.substr(1, len-1);
		gotoPath.append(homePath);
		gotoPath.append(sub);
		gotoPath.append("/");
	}
	pathStack.push(currPath);
	currPath = gotoPath;
	fileVec.clear();
	fileCount = clearScreenAndListFiles((char *)gotoPath.c_str());
	winIndex = 0;
	moveCursorTop();
	return 0;
}

void clearCommandVecAndBottomScreen(){
	cVec.clear();
...
int normalModedriver(int argc, char **argv){
	...
	homePath = currPath;
	ioctl(fileno(stdout), TIOCGWINSZ, &wSize);		//to get the size of the current terminal
	winLen = wSize.ws_row;
	...
	while(true){
		...
		if(ch == KEY_ESCAPE){
			...
			if(ch == KEY_BACKSPACE){
				...
				return -1;		//should be used later once functionality is build
			}
		} else if(ch == ' '){
			cVec.push_back(arg);
			arg.clear();			
...
int commandHandler(vector<string> vec){
	...
	if(command == "copy"){
		...
		//fileCount = clearScreenAndListFiles((char *)currPath.c_str());
		clearCommandVecAndBottomScreen();
		...
		status = createFileHandler(size, vec, currPath);
		fileCount = clearScreenAndListFiles((char *)currPath.c_str());
		clearCommandVecAndBottomScreen();
	} else if(command == "create_dir"){
		status = createDirHandler(size, vec, currPath);
		fileCount = clearScreenAndListFiles((char *)currPath.c_str());
		clearCommandVecAndBottomScreen();
	} else if(command == "delete_file"){
		status = deleteFileHandler(size, vec, currPath);
		fileCount = clearScreenAndListFiles((char *)currPath.c_str());
		clearCommandVecAndBottomScreen();
	} else if(command == "goto"){
		status = gotoSpecificDirectory(vec[1]);
		clearCommandVecAndBottomScreen();
	} /*else if(command == "search"){
		vector<string> searchVec = searchHandler (vec.size(), vec, currPath); 
		clearScreen();														//clear the screen
		printf("\033[%d;%dH",iRow,iCol);
		for(string path :  searchVec){
			printf("%s\n",path);
		}
		clearCommandVecAndBottomScreen();
		winIndex = 0;
